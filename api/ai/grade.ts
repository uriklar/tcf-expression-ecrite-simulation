import { handleGradeRequest } from './_handlers.js';
...
export default async function handler(request: JsonRequest, response: JsonResponse) {
  try {
    ...
    const result = await handleGradeRequest(request.method, body, apiKey);
    sendJson(response, result.status, result.body);
  } catch (error) {
    console.error('AI grade API failed', toLoggableError(error));
    sendJson(response, error instanceof SyntaxError ? 400 : 500, {
      error: error instanceof SyntaxError ? 'Invalid JSON request.' : 'A server error occurred while checking this answer.',
    });
  }
}
